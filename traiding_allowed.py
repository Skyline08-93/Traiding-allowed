...
async def find_triangles(symbols):
    ...
    for base in start_coins:
        # Ищем пары, содержащие базовую валюту в начале или конце
        base_pairs = [s for s in symbols if s.endswith(base) or s.startswith(base)]
        
        for pair in base_pairs:
            # Определяем формат пары: XXXbase или baseXXX
            if pair.endswith(base):
                coin1 = pair[:-len(base)]
                pair_format = "direct"  # XXXbase
            else:
                coin1 = pair[len(base):]
                pair_format = "inverse"  # baseXXX
            
            # Ищем пары, содержащие coin1
            for candidate in symbols:
                if candidate == pair:
                    continue
                    
                # Проверяем все возможные комбинации
                if candidate.startswith(coin1):
                    coin2 = candidate[len(coin1):]
                    # Проверяем завершающую пару
                    for final_pair in [coin2 + base, base + coin2]:
                        if final_pair in symbols and final_pair != pair:
                            triangles.append((base, coin1, coin2))
                elif candidate.endswith(coin1):
                    coin2 = candidate[:-len(coin1)]
                    # Проверяем завершающую пару
                    for final_pair in [coin2 + base, base + coin2]:
                        if final_pair in symbols and final_pair != pair:
