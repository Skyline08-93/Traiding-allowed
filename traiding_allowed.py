# === traiding_allowed.py (обновленная версия) ===
...
async def execute_real_trade(route_id, steps, base_coin, markets):
    try:
        dynamic_volume = steps[0][3]  # Объем из первого шага
        
        await send_telegram_message(f"🚀 Запуск торговли по маршруту: {route_id} | Объем: ${dynamic_volume:.2f}")
        print(f"[ТОРГОВЛЯ] Маршрут: {route_id} | Объем: ${dynamic_volume:.2f}")
        ...
        if free_balance < dynamic_volume:
            msg = f"❌ Недостаточно {base_coin}. Доступно: {free_balance:.2f}, нужно: {dynamic_volume:.2f}"
            await send_telegram_message(msg)
...
Маршрут: {route_id}
Начальный объем: ${dynamic_volume:.2f}
...
async def check_triangle(base, mid1, mid2, symbols, markets):
    try:
        ...
        # Первоначальное сканирование ликвидности
        ...
        # Определяем объем сделки (90% от минимальной ликвидности, но в пределах $10-$100)
        min_liquidity = min(liq1, liq2, liq3)
        dynamic_volume = min(
            max(min_liquidity * 0.9, min_trade_volume),  # 90% от доступной ликвидности, но не менее $10
            max_trade_volume  # И не более $100
        )

        # Пересчитываем с точным объемом
        ...
        pure_profit_usdt = round((result - 1) * dynamic_volume, 2)
        balances = await fetch_balances()
        asset = base.split(":")[0].replace(":", "")
        free_amount = balances.get(asset, 0)
        
        if free_amount < dynamic_volume:
            msg = f"❌ Недостаточно {asset} на балансе. Доступно: {free_amount:.2f} {asset}, нужно: {dynamic_volume:.2f}"
            if debug_mode:
        ...
        message = "\n".join([
            ...
            f"💧 Мин. ликвидность на шаге: ${min(liq1, liq2, liq3):.2f}",
            f"📊 Объем сделки: ${dynamic_volume:.2f}",
        ...
        log_route(base, mid1, mid2, profit_percent, dynamic_volume)
