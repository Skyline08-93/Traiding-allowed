...
async def find_triangles(symbols):
    """Исправленная функция для поиска треугольников на Bybit"""
    triangles = []
    for base in start_coins:
        # Фильтруем пары, заканчивающиеся на базовую валюту (USDT, BTC, ETH)
        base_pairs = [s for s in symbols if s.endswith(base)]
        
        for pair1 in base_pairs:
            # Извлекаем первую валюту из пары (например, BTC из BTCUSDT)
            coin1 = pair1.replace(base, "")
            
            # Ищем пары, начинающиеся с coin1
            for pair2 in symbols:
                if pair2.startswith(coin1) and not pair2.endswith(base):
                    # Извлекаем вторую валюту (например, ETH из BTCETH)
                    coin2 = pair2.replace(coin1, "")
                    
                    # Формируем обратную пару (например, ETHUSDT)
                    pair3 = coin2 + base
                    
                    # Проверяем существование обратной пары
                    if pair3 in symbols:
                        triangles.append((base, coin1, coin2))
    return triangles
...
async def check_triangle(base, mid1, mid2, symbols, markets):
    try:
        ...
        # Формируем пары в формате Bybit (без разделителя)
        s1 = mid1 + base    # BTCUSDT
        s2 = mid1 + mid2    # BTCETH
        s3 = mid2 + base    # ETHUSDT

        # Проверка существования пар на бирже
        valid_pair = True
        for pair in [s1, s2, s3]:
            if pair not in markets:
                if debug_mode:
                    print(f"[⚠️] Пара {pair} не найдена на бирже")
                valid_pair = False

        # Фиксированные направления для Bybit-формата
        trade_side1 = 'buy'   # Покупаем базовую валюту (BTC за USDT)
        trade_side2 = 'buy'   # Покупаем вторую валюту (ETH за BTC)
        trade_side3 = 'sell'  # Продаем вторую валюту (ETH за USDT)
        ...
        if trade_ready and not is_shutting_down:
            ...
            if real_trading_enabled:
                trade_steps = [
                    (s1, trade_side1, price1, target_volume_usdt),
                    (s2, trade_side2, price2, target_volume_usdt / price1),
                    (s3, trade_side3, price3, target_volume_usdt / price1 / price2)
                ]
...
async def main():
    try:
        ...
        if debug_mode:
            ...
            print(f"Пример маршрута: {triangles[0] if triangles else 'нет'}")
