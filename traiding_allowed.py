    ...
    try:
        ...
        for i, (symbol, side, price, amount) in enumerate(steps, 1):
            try:
                ...
                # 🔁 Новый способ проверки исполнения через closed_orders
                ...
                if matched_order is None:
                    ...
                    await send_telegram_message(f"❌ Шаг {i}: Ордер не исполнен или не найден — {symbol} {side}")
                    return False
                ...
                if filled == 0:
                    await exchange.cancel_order(order["id"], symbol)
                    await send_telegram_message(f"❌ Шаг {i}: Ордер не исполнен (filled = 0) — {symbol} {side}")
                    return False
